...
class MotionLoader:
  def __init__(
    self, motion_file: str, body_indexes: torch.Tensor, device: str = "cpu", compute_kinetic_energy = False
  ) -> None:
...
class MotionCommand(CommandTerm):
  ...
  def __init__(self, cfg: MotionCommandCfg, env: ManagerBasedRlEnv):
    ...
    self.motion = MotionLoader(
      self.cfg.motion_file, self.body_indexes, device=self.device, compute_kinetic_energy = (self.cfg.sampling_mode == "lke")
    )
