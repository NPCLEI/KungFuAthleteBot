...
try:
    ...
    append_metrics_jsonl = None  # type: ignore[assignment,misc]
...
class LoggingHelper:
    def __init__(
        ...
        self.console_logging_interval: int = max(1, console_logging_interval)
    ...
    def post_epoch_logging(
        ...
        env_log_dict_raw = self.episode_env_tensors.mean_and_clear()
        env_log_dict = {f"Env/{k}": v for k, v in env_log_dict_raw.items()}
        for k, v in env_log_dict_raw.items():
            env_log_dict.setdefault(k, v)
        env_log_dict = self._all_reduce_scalar_dict(env_log_dict)
        ...
        # Rich Live on every rank can interleave with tee/torchrun and stall other ranks.
        if self.is_main_process and it % self.console_logging_interval == 0:
        ...
        if self.ep_infos:
            for key in self.ep_infos[0]:
                ...
                if key.startswith("rew_"):
                    scalars_to_log[f"Episode_Reward/{key[4:]}"] = value
                elif "/" in key:
    ...
    def _logging_to_writer(
        ...
        scalars_to_log["Train/total_timesteps"] = self.tot_timesteps

        # Add prefix to all keys
        scalars_to_log = {f"{self.prefix}{k}": v for k, v in scalars_to_log.items()}
        scalars_to_log = expand_ppo_compatible_keys(scalars_to_log)
        ...
        if append_metrics_jsonl is not None:
            append_metrics_jsonl(self.log_dir, it, scalars_to_log)

    def _all_reduce_scalar_dict(self, data: dict[str, float]) -> dict[str, float]:
        if not torch.distributed.is_available():
            return data
        if not torch.distributed.is_initialized() or self.num_gpus <= 1:
            return data

        # Union keys across ranks; skipping all_reduce when local dict is empty deadlocks.
        key_lists: list[list[str] | None] = [None for _ in range(self.num_gpus)]
        torch.distributed.all_gather_object(key_lists, sorted(data.keys()))
        all_keys = sorted({key for keys in key_lists if keys for key in keys})
        if not all_keys:
            return data
        ...
        for key in all_keys:
            ...
            reduced[key] = tensor.item() / self.num_gpus
        return reduced
