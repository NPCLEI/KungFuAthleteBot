...
from holosoma_min.agents.base_algo.base_algo import BaseAlgo
from holosoma_min.agents.callbacks.base_callback import RLEvalCallback
from holosoma_min.agents.fast_sac.fast_sac import Actor, CNNActor, CNNCritic, Critic
from holosoma_min.agents.fast_sac.fast_sac_utils import (
    EmpiricalNormalization,
    SimpleReplayBuffer,
    save_params,
)
from holosoma_min.agents.modules.augmentation_utils import SymmetryUtils
from holosoma_min.agents.modules.logging_utils import LoggingHelper
from holosoma_min.config_types.algo import FastSACConfig
from holosoma_min.envs.base_task.base_task import BaseTask
from holosoma_min.utils.average_meters import TensorAverageMeterDict
from holosoma_min.utils.helpers import instantiate
from holosoma_min.utils.inference_helpers import (
...
from holosoma_min.utils.safe_torch_import import (
    F,
    GradScaler,
...
class FastSACAgent(BaseAlgo):
    ...
    def __init__(
        ...
        self.logging_helper = LoggingHelper(
            ...
            console_logging_interval=config.console_logging_interval,
        )
    ...
    def setup(self) -> None:
        try:
            from src.rl.training.mujoco_stderr_monitor import install_overflow_stderr_tap

            install_overflow_stderr_tap()
        except ImportError:
            pass
    ...
    def _all_reduce_model_grads(self, model: nn.Module) -> None:
        ...
        if not params_with_grad:
            # All ranks must enter all_reduce; use zero grads when local backward skipped.
        ...
        if self.config.use_autotune:
            ...
            if self.is_multi_gpu:
                alpha_grad = (
                    ...
                    else torch.zeros_like(self.log_alpha.data)
                )
                torch.distributed.all_reduce(alpha_grad, op=torch.distributed.ReduceOp.SUM)
                alpha_grad.div_(self.gpu_world_size)
                if self.log_alpha.grad is not None:
                    self.log_alpha.grad.data.copy_(alpha_grad)
    ...
    def learn(self) -> None:
        ...
        pbar = (
            tqdm.tqdm(total=args.num_learning_iterations, initial=self.global_step)
            if self.is_main_process
            else None
        )

        try:
            from src.rl.training.mujoco_stderr_monitor import (
                check_divergence_early_stop,
                reset_overflow_step_counter,
            )
        except ImportError:
            check_divergence_early_stop = None  # type: ignore[assignment,misc]
            reset_overflow_step_counter = None  # type: ignore[assignment,misc]

        while self.global_step <= args.num_learning_iterations:
            if reset_overflow_step_counter is not None:
                reset_overflow_step_counter()
            ...
            with self.logging_helper.record_collection_time():
                ...
                with torch.no_grad():
                    overflow_n = 0
                    try:
                        from src.rl.training.mujoco_stderr_monitor import overflow_lines_this_step
                        from src.rl.training.reward_anomaly_logger import log_reward_anomaly_from_agent

                        overflow_n = overflow_lines_this_step()
                        log_reward_anomaly_from_agent(
                            step=self.global_step,
                            rewards=rewards,
                            agent=self,
                            overflow_lines=overflow_n,
                        )
                    except ImportError:
                        r = rewards.float()
                        bad = r.abs() > 1e4
                        if bad.any() or not r.isfinite().all():
                            logger.warning(
                                f"step={self.global_step} reward stats: "
                ...
                with torch.no_grad():
                    bad_mask = ~torch.isfinite(rewards)
                    bad_mask |= ~torch.isfinite(obs).all(dim=-1)
                    bad_mask |= ~torch.isfinite(true_next_obs).all(dim=-1)
                    bad_mask |= ~torch.isfinite(critic_obs).all(dim=-1)
                    bad_mask |= ~torch.isfinite(true_next_critic_obs).all(dim=-1)
                    if bad_mask.any():
                        bad_ids = bad_mask.nonzero(as_tuple=False).squeeze(-1)
                        if bad_ids.numel() > 0:
                            logger.warning(
                                f"step={self.global_step} skipping bad replay transitions "
                                f"for env_ids={bad_ids.tolist()[:16]} "
                                f"(count={bad_ids.numel()})"
                            )
                        transition["observations"] = torch.nan_to_num(
                            transition["observations"], nan=0.0, posinf=0.0, neginf=0.0
                        )
                        transition["critic_observations"] = torch.nan_to_num(
                            transition["critic_observations"], nan=0.0, posinf=0.0, neginf=0.0
                        )
                        transition["next"]["observations"] = torch.nan_to_num(
                            transition["next"]["observations"], nan=0.0, posinf=0.0, neginf=0.0
                        )
                        transition["next"]["critic_observations"] = torch.nan_to_num(
                            transition["next"]["critic_observations"],
                            nan=0.0,
                            posinf=0.0,
                            neginf=0.0,
                        )
                        transition["next"]["rewards"] = torch.where(
                            bad_mask,
                            torch.zeros_like(rewards),
                            transition["next"]["rewards"],
                        )
                        transition["next"]["dones"] = torch.where(
                            bad_mask,
                            torch.ones_like(transition["next"]["dones"]),
                            transition["next"]["dones"],
                        )
            ...
            step_loss_dict: dict[str, float] = {}
            if self.global_step > args.learning_starts:
                with torch.no_grad():
                    accumulated_metrics = self.training_metrics.mean_and_clear()
                    for key, value in accumulated_metrics.items():
                        if isinstance(value, torch.Tensor):
                            step_loss_dict[key] = value.item()
                        else:
                            step_loss_dict[key] = float(value)

            step_loss_dict["env_rewards"] = rewards.mean().item()
            step_loss_dict["actor_learning_rate"] = args.actor_learning_rate
            step_loss_dict["critic_learning_rate"] = args.critic_learning_rate
            step_loss_dict["alpha_learning_rate"] = args.alpha_learning_rate
            step_loss_dict["learning_rate"] = args.actor_learning_rate

            if self.global_step % args.logging_interval == 0:
                self.logging_helper.post_epoch_logging(
                    it=self.global_step, loss_dict=step_loss_dict, extra_log_dicts={}
                )
                self._distributed_barrier()

            if check_divergence_early_stop is not None and getattr(args, "early_stop_enabled", False):
                stop_reason = check_divergence_early_stop(
                    step_loss_dict,
                    enabled=True,
                    abs_reward_threshold=args.early_stop_abs_reward,
                    check_nan_loss=args.early_stop_nan_loss,
                    overflow_lines_threshold=args.early_stop_overflow_lines_per_step,
                )
                if stop_reason is not None:
                    if self.is_main_process:
                        ckpt = os.path.join(self.log_dir, f"early_stop_step_{self.global_step:07d}.pt")
                        logger.warning(f"Early stop at step {self.global_step}: {stop_reason}")
                        self.save(ckpt)
                    self._distributed_barrier()
                    break
            ...
            if args.save_interval > 0 and self.global_step > 0 and self.global_step % args.save_interval == 0:
                if self.is_main_process:
                    ...
                    try:
                        ...
                        logger.warning(f"Skipping built-in ONNX export: {export_exc}")
                self._distributed_barrier()
    ...
    def save(self, path: str) -> None:  # type: ignore[override]
        ...
        save_params(
            ...
            metadata=(
                ...
                else {"iteration": int(self.global_step)}
            ),
        )
    ...
    def export(self, onnx_file_path: str) -> None:
        ...
        was_norm_training = (
            self.obs_normalizer.training if self.obs_normalization else None
        )
        ...
        try:
            ...
            if was_training:
                self.actor.train()
            if self.obs_normalization and was_norm_training:
                self.obs_normalizer.train(was_norm_training)
