...
class FastSACConfig:
    ...
    """Rich console panel interval (TensorBoard/JSONL use logging_interval)."""

    early_stop_enabled: bool = False
    """Stop training when divergence / MuJoCo overflow burst is detected."""

    early_stop_abs_reward: float = 1e4
    """Early stop if |env_rewards| or |buffer_rewards| exceeds this."""

    early_stop_nan_loss: bool = True
    """Early stop on non-finite actor/qf/alpha loss."""

    early_stop_overflow_lines_per_step: int = 50
    """Early stop if stderr overflow lines in one global step exceed this."""
